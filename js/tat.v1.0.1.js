(function() {
	...
	QQTAT.lib = {
		...
		"$Tips" : {
			...
			"show" : function(sText, from) {
				...
				this.eMask.style.backgroundImage = "url(http://mat1.gtimg.com/www/a11y/c.png)";
		...
		$fixed : {
			...
			"fScrollDone" : function() {
				var dPanel = QQTAT.lib.$Id("a11y_TAT_Panel"), dSubPanel = QQTAT.lib.$Id("a11y_TAT_Subtitle_Panel"), oTarget = QQTAT.lib.$fixed.oTarget, nZoomMult = QQTAT.lib.$fixed.nZoomMult;
			...
			"init" : function(oWho, oTarget, nZoomMult, bCanncel) {
				var oLib = QQTAT.lib, dPanel = oLib.$Id("a11y_TAT_Panel"), dSubPanel = oLib.$Id("a11y_TAT_Subtitle_Panel");
	...
	QQTAT.sWitch = {
		...
		cursor : function(sSwitch, sIsLoading) {
			var sTatStyle = 'body{cursor:url(http://mat1.gtimg.com/www/a11y/aero_arrow_xl.cur),auto !important}a{cursor:url(http://mat1.gtimg.com/www/a11y/aero_link_xl.cur),auto !important}';
			var $Lib = QQTAT.lib;
			if (sSwitch) {
				if (sIsLoading) {
	...
	QQTAT.zoom = {
		zoom : function(m) {
			var d = false;
			var l = QQTAT.lib.$Tag("body")[0], k = l.offsetWidth, allyHook = QQTAT.lib.$Id("a11y_TAT_Hook"), ally = QQTAT.lib.$Id("a11y_TAT_Panel"), allySub = QQTAT.lib.$Id("a11y_TAT_Subtitle_Panel"), panelMult = "scale(" + (1 / m) + ")", transOrigin = k * (m - 1) / 2 + "px top", bodyMult = "scale(" + m + ")";
		...
		zoomIn : function() {
			var oSelf = QQTAT.zoom, config = QQTAT.config, aMult = config.zooMmultiples;
		...
		Revert : function() {
			var oSelf = QQTAT.zoom, config = QQTAT.config;
			if (config.zoomCurr == 0) {
	...
	QQTAT.help = {
		"init" : function() {
			var eHelp = QQTAT.lib.$Id("a11y_TAT_Help"), eCopyRight = QQTAT.lib.$Id("a11y_TAT_Copyrighs"), oSelf = this;
	...
	QQTAT.subtitle = {
		...
		"sound" : function(txt) {
			...
			if (QQTAT.lib.$Browser.mozilla || QQTAT.lib.$Browser.opera) {
				...
				if ( typeof FlashObject == "undefined") {
					QQTAT.lib.$loadedJs("http://mat1.gtimg.com/www/a11y/flash/swfobject.js", function() {
						var so = new FlashObject("http://mat1.gtimg.com/www/a11y/flash/QtPlayMp3.swf", "soundPlayer", "0", "0", "0", "#ffffff");
		...
		"createTat" : function(txt) {
			var fragment = document.createDocumentFragment(), ele, $C = QQTAT.lib.$Create, $CT = QQTAT.lib.$CreateText;
		...
		"tatOver" : function(e) {
			var ele = e.target || e.srcElement, i, j, str = '';
			...
			for ( i = 0; i < ele.childNodes.length; i++) {
				var node = ele.childNodes[i], parent = node.parentNode, html = '', arr;
				if (node.nodeType == 3) {
	...
	QQTAT.boss = {
		bossFn : function(name) {
			var _uin = document.cookie.match(new RegExp('(^|)o_cookie=([^;]*)(;|$)')), uin = (_uin == null ? "" : unescape(_uin[2]));
		...
		bind : function() {
			QQTAT.lib.$AddEvent(QQTAT.lib.$Id('a11y_TAT_Hook'), 'mouseover', function() {
